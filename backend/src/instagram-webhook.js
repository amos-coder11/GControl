...
/** Envía un audio / nota de voz por Instagram Messaging (URL pública HTTPS). */
export async function sendInstagramAudio({ igsid, audioUrl }) {
  ...
  if (!url) {
    const err = new Error("audio_url_required");
    err.code = "audio_url_required";
    throw err;
  }
  ...
  for (const attempt of attempts) {
    try {
      ...
      console.warn("[instagram/send-audio] attempt failed", lastError.message);
    } catch (err) {
      lastError = {
        status: 0,
        details: null,
